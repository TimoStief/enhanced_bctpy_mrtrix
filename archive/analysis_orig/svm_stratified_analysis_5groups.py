...
def run_stratified_analysis(df_merged, df_encoded, feature_cols):
    ...
    if comparison_data:
        ...
        df_comparison.to_csv(OUTPUT_DIR / 'stratified_sex_group_comparison_5groups.csv', index=False)
        print(f"\n✅ Saved to stratified_sex_group_comparison_5groups.csv")
...
def save_results(results_summary):
    ...
    with open(OUTPUT_DIR / 'stratified_analysis_summary_5groups.json', 'w') as f:
    ...
    print(f"   - stratified_analysis_summary_5groups.json")
    print(f"   - stratified_sex_group_comparison_5groups.csv")
